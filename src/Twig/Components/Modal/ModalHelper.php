...
namespace CyclingApps\ComponentBundle\Twig\Components\Modal;
...
class ModalHelper
{
    ...
    public function render(string $backgroundUri, string $template, array $context = []): Response
    {
        ...
        if ($request->query->get('_cyclingapps_component_modal')) {
            return new Response($modal);
        }

        $subRequest = Request::create($backgroundUri);
        $subRequest->attributes->set('_cyclingapps_component_modal', ['content' => $modal, 'backgroundUri' => $backgroundUri]);
